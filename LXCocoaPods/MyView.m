...
////确定颜色的值，将颜色计数的值存到数组里默认为0，即为绿色
//static UIColor * currentColor;
////确定宽度的值，将宽度计数的值存到数组里默认为0，即为10
//static CGFloat currentWidth;
//
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([lineArray count]>0) {
        ...
        if ([array count]>0)
        {
            ...
            float width= [WidthArray[i] floatValue];
    ...
    if ([pointArray count]>0)
    {
        ...
        UIColor *lineColor= currentColor;
...
-(void)addLA{
    ...
    NSNumber * num = [NSNumber numberWithFloat:currentWidth];
